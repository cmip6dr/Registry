...
%% Using MOF: https://link.springer.com/chapter/10.1007/978-3-319-67271-7_7 (need to buy/order).  Model Driven Architecture
...
The CMIP6 Data Request \citep[DREQ][]{juckes2020} provides detailed technical specifications of thousands of
...
The ability to discuss the whole range of issues from organisational structure to comprehensive technical details is a valuable 
...
In addition to these ISO standards we will draw on the OGC standards SKOS and PROV {\color{red} EXPAND}.
...
a systematic approach to collecting provenance information in a distributed software system.
...
Setting out the requirements for provenance collection clearly in the upper layers provides a systematic framework for embedding the functionality consistently in a range of different artefacts at the workflow implementation layer. 


The ISO standards are developed in order to promote interoperability between institutions (e.g. Sinaci et al. 
\cite{sinaci2013} describe the use of MDR to facilitate exchange across clinical work and care domains), but also
provide what could be considered a guide to best-practice and a software independent representation that can aid design and
provide interoperability of design across system upgrades.


\subsection{More about the data request}

The DREQ is part of a range of infrastructure projects \citep{Balaji2018} which have evolved to support
the growing demands of the CMIP process. See also ES-DOC \citep{pascoe2019disc}.

The design of the Data Request implemented for CMIP6 \citep[DREQ][]{juckes2020} can be viewed as a 3-layer approach, with layers characterised as
``framework'', ``configuration'' and ``content'' which can be mapped, with some adjustment, onto the lower 3-layers of a MOF system.
...
The ``content'' section, which will become the ``delivery'' layer, contains the final metadata records which define the physical parameters which are requested from 
CMIP participants, and also contains user facing web pages and software tools designed for use by the user community. 
The metadata records are published as an XML database conforming to an XSD schema, which is specified in the ``configuration'' section. 


The DREQ is built on domain standards and practices which have evolved with the CMIP project, some of which have not been formally documented in
a systematic way [is this fair?]. The layered approach allows these standards to be incorporated and clearly represented in the ``Definition'' and ``Design'' stages. 
...
pre-existing structures, so that parts of the model serve as formalised representations of an existing system rather than a design specification. 
...
the four layer model described in the review by Djuric et al.\cite{djuric2006} in which the layers are:
...
\subsubsection{MOF Factories and Instantiation}
...
\item \cd{Imports of Standards and Protocols}
\end{itemize}

\subsubsection{Parameter Registration}
When a user group proposes a new parameter it will be registered as an item for discussion almost automatically, but deeper discussion may lead to it being considered
redundant (e.g. covered by a pre-existing term or a term being registered through a parallel discussion) or 
broken into multiple requests. 

\subsubsection{Technical Construct Registration}

The definitions of parameters are complemented by a range of technical specifications, such as masking by land cover types or physical dimensions such as radiation wavelength or snow depth. This type of information is designed for programmatic consumption
by the software packages which are used to write and read the archive data. For efficient generation of data it is particularly important that any changes in this area are compatible with the software packages used for data generation, especially CMOR. At the same time, the technical details may be somewhat opaque to scientific users. This leads to a different range fo discussions for these decisions.

\subsubsection{Imports of Standards and Protocols}

As noted earlier ({\color{red} Section ??}), the DREQ is tied to some co-evolving standards and protocols, and needs to reflect and 
benefit from updates to these external sources. Imports cannot always be done automatically, as there may require either additional
information  or adjustments to linked content. Hence, a review and decision process is required. This can be complex if, as happened early in the CMIP6 process, it is not just the content of the different protocols which is evolving, but also the syntax. 
With advancing matirity of different infrastructure components this should become easier.
...
\subsection{Defining physical parameters}


The definitions used for physical parameters in the Data Request draw on definitions set within the CF Convention. This
relationship to the CF Conventions is a fundamental aspect of the Data Request which underpins the communication of
technical information and enables wide use of the data products based on the data request. The comparison with
approaches used in other standards is not, therefore, goint to affect the way in which definitions are represented in
the data request. It can, however, provide some ideas for improving clarity of presentation.

The lists of thousands of variables in the Data Request are daunting for many users. MDR suggests 
defining physical quantities in terms of \cd{MDR_Object}, \cd{MDR_Property}, \cd{MDR_Conceptual_Domain}.
If we are describing a red car, for instance, ``car'' is the object and ``red (color)'' is the property. The split 
...
The problem of rationalising an approach to categorising concepts has been discussed extensively by Ranganathan ({\color{red} REFERENCES})
in papers developing his PMEST (Personality, Matter, Energy, Space, Time) categorisation scheme (the last four of these may have a familiar ring to physical scientists, but the intented meaning of ``matter'' and ``energy'' here is much broader than the physical science terms). 

The QU concept of \cd{quantity} is closely associated with dimensionality, which is an attribute of \cd{MDR_Conceptual_Domain}.
We also consider this as the \cd{CC_Personality} element of PMEST. The \cd{CC_Matter} element can be aligned with \cd{MDR_Object}, leaving \cd{MDR_Property} to represent \cd{CC_Energy}.

Thus, for \cd{atmosphere_mass_content_of_convective_cloud_condensed_water} we have:
\cd{MDR_Object:atmospheric_convective_cloud_water}, \cd{MDR_Property:condensed}, \cd{MDR_Conceptual_Domain:mass_content}.

See table \ref{tab:qu}.

The first three of these can be considered as eqivalent \cd{MDR_Object}, \cd{MDR_Property} and \cd{MDR_Processing}.

Ranganathan is credited with introducing a facet based approach: his classification evolved into the PMEST

CF Ranganathan, S. R. Ranganathan 
...
Personaility is the primary facet, but defined most clearly as inate characteristics which are not to do with physical make-up (matter) or processes (energy). 

The first three components of the PMEST formalisation can be aligned with MDR concepts of object, property and process. This does not
resolve the question of partitioning between object and property: is the cloud a property of the atmosphere or an object? The QU standard does define, through enumeration, a well developed concept of quantity, in terms of physical attributes. 

BUT ... ``property'' is a distinquishing feature .... it does not really go with "quantity". DMR specifies a "dimensionality". Hence, if "cloud" is an "object", "convective" could be a "MDR_property". 


Here, the \cd{MDR_Object} absorbs considerable detail, leaving relatively simple concepts in \cd{MDR_Property} and \cd{MDR_Conceptual_Domain}.

[[potential also to use \cd{MDR_Data_element_derivation}.

-- only allowed one property ...

-- The potential issue here is the dual use of "property" for (i) the ocnvective nature of the cloud and (ii) the condesed water component of the the cloud.

MDR Conceptual Domain could be aligned with quantity. This has a dimensionality ... 

Broughton: https://doi.org/10.1080/13614560108914727
