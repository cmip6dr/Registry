...
\umlsimpleclass[sc1,y=-1,x=9,type={EIC}]{Organisation}
\umlsimpleclass[sc1,y=-3,x=2,type={mdr}]{Register}

\umlcompo[link1,arg2={Functional Entity},align2=left,pos2=0.95,stereo={resource}, anchor stereo=north]{Organisation}{Registry}
\umlcompo[link1,arg2={Organisational Element},align2=right, mult2=*]{Registry}{Register}
...
different kinds of modeling. In engineering, for instance, the lowest level may refer to physical objects which have been created.
This article [add ref], uses four layers characterised as
``real world'', ``model'', ``meta-model'', ``MOF''.
...
This article [add ref] has the following four layers:
``application'', ``Semantic'', ``Object'', ``Syntax''. [http://infolab.stanford.edu/~melnik/pub/sw00/]
%% A Layered Approach to Information Modeling and Interoperability on the Web 2000-30.pdf

\cite{mcclatchkey2018} discusses the use of a MOF layered metamodel to design a systematic approach to collecting provenance information in a distributed software system.
...
\subsubsection{The MOF Profile}

MOF provides a flexible framework for modeling, even allowing flexibility around the number of layers. OMG2014 suggest two layers for
software design, three for a database. Here we wish to describe software, database, and also the surrounding organisation and interactions.
...
Hence, we adopt four layers. In many introductions to MOF, the four layers are identified as Real World, Model, Meta-Model and Meta-meta-model or MOF layer.
%http://www.jot.fm/issues/issue_2006_11/article4/article4.pdf
Real World -- model -- meta-model -- MOF; model layers --- all except M0, expressible in XMI. 
\cite{djuric2006}
For instance, if applied to warehouse construction, the meta-model might be a statement of technical requirements given to a design team,
the model would be the design document produced by the design team,
and the final stage would be a construction company comissioned to build the warehouse.

In software engineering projects it is common practice to identify four MOF layesr as representing:
Application - Semantic - Object - Syntax [http://infolab.stanford.edu/~melnik/pub/sw00/]
%% A Layered Approach to Information Modeling and Interoperability on the Web 2000-30.pdf -- grey lit ...
Here, the software application stands in for the ``real world'' object, and the semantic description of the software is considered
as a ``model''. 

In both these MOF Profiles, the people responsible for implementing the transitions between one model layer and the next are,
intentionally, outside the model. 

\cite{inan2018} model a disaster response system which brings some of the people, called ``agents'' in this context, into the
model.
They follow the common terminology, referring to M0 as ``real world'' and M1 as ``model'' layers, but, in an interesting twist,
...
For the meta-data registry application considered here it is important to represent the agents responsible for
...
We will also be representing active agents in the M1 layer, and even in the M2 layer. In order to avoid the unfortunate connotation that these are in some way un-real, we introduce new terminology for the layers:
Delivery -- Design -- Requirements -- Concept

\cite{inan2018} Inan et al.\ consider M0 as the ``real world'' layer, M1 as the model layer, and M2 as meta-model.
In their agent-based approach, there are, nevertheless agents and goals defined in the model layer.
The goals are abstract or managerial in nature, rather than relating to specific problems.
...
EIF describes a seven enterprise phases which can be aligned with the MOF layers. Domain Identification and Concept Definition will both be covered by the Concept layer. Requirements Definition will, naturally, be in the Requirements layer. Design specification and implementation
description will be captured in the design layer, with the functional design elements specified as classes and the implementation rules
defined through MOF Factory elements.
...
Management of provenance information is a critical part of the devlopment and maintenance of standards. This is a significant challenge here
because the development work can involve staff from hundreds of institutions working in ad-hoc teams. We wish to capture the contributiomns
